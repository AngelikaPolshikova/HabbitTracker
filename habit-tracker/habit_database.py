...
def update_last_checked_date(db, name, checked_at):
    # Update the last checked date for a habit in the count table
    cur = db.cursor()
    cur.execute("UPDATE count SET Checked_at=? WHERE name=?", (checked_at, name))
    db.commit()
